...
namespace Thryft.Services.Tests
{
    ...
    public class UserServiceTests
    {
        ...
        public async Task InitializeAsyncTest_WithStoredUser_SetsCurrentUser()
        {
            ...
            // Mock the GetAsync to return a successful result
            _mockProtectedLocalStorage
                .Setup(x => x.GetAsync<User>("currentUser"))
                .ReturnsAsync((ProtectedBrowserStorageResult<User>)CreateMockBrowserStorageResult(storedUser, true));
        ...
        public async Task InitializeAsyncTest_NoStoredUser_CurrentUserRemainsNull()
        {
            ...
            _mockProtectedLocalStorage
                .Setup(x => x.GetAsync<User>("currentUser"))
                .ReturnsAsync((ProtectedBrowserStorageResult<User>)CreateMockBrowserStorageResult<User>(default, false));
        ...
        // Helper method using dynamic to create the result
        private dynamic CreateMockBrowserStorageResult<T>(T value, bool success)
        {
            // Create an anonymous type with the same structure
            return new
            {
                Success = success,
                Value = value
        ...
        [TestMethod()]
        public async Task InitializeAsyncTest_CryptographicException_DeletesStoredUser()
        {
            // Arrange
            _mockProtectedLocalStorage
                .Setup(x => x.GetAsync<User>("currentUser"))
                .ThrowsAsync(new System.Security.Cryptography.CryptographicException("Invalid key"));
        ...
        [TestMethod()]
        public async Task MigrateExistingPasswordsTest_MigratesNonBcryptPasswords()
        {
            // Arrange
            var usersWithOldPasswords = new List<User>
            {
                new User { UserId = 1, Email = "user1@example.com", Password = "plaintext1" },
                new User { UserId = 2, Email = "user2@example.com", Password = "$2a$10$hashed" }, // Already bcrypt
                new User { UserId = 3, Email = "user3@example.com", Password = "plaintext2" }
            };

            var mockUsersDbSet = usersWithOldPasswords.AsQueryable().BuildMockDbSet();
            ...
            mockContext.Setup(c => c.SaveChangesAsync(It.IsAny<System.Threading.CancellationToken>()))
                .ReturnsAsync(1);
            ...
            // Act
            await _userService.MigrateExistingPasswords();

            // Assert
            mockContext.Verify(x => x.SaveChangesAsync(It.IsAny<System.Threading.CancellationToken>()), Times.Once);

            // Users with plaintext passwords should be hashed
            Assert.IsTrue(usersWithOldPasswords[0].Password.StartsWith("$2"));
            Assert.AreEqual("$2a$10$hashed", usersWithOldPasswords[1].Password); // Already hashed remains same
            Assert.IsTrue(usersWithOldPasswords[2].Password.StartsWith("$2"));
        }
    }
    ...
    public static class TestExtensions
    {
        public static Mock<DbSet<T>> BuildMockDbSet<T>(this IQueryable<T> data) where T : class
        {
            ...
            mockSet.Setup(m => m.Update(It.IsAny<T>())).Callback<T>(item =>
            {
                ...
                var existing = list.FirstOrDefault(i => i.Equals(item));
                if (existing != null)
                {
